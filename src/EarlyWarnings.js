...
function EarlyWarnings({ onClose, userRole }) {
  ...
  <div style={{ display: 'flex', flexDirection: 'column', gap: '8px', padding: '10px 12px', border: '0.5px solid #b4b2a9', borderRadius: '6px', background: '#fff' }}>
    {[
      ...
      <label key={option.value} style={{ display: 'flex', alignItems: 'center', gap: '8px', fontSize: '13px', cursor: 'pointer' }}>
        <input
          type="checkbox"
          checked={form.risk_type.includes(option.value)}
          onChange={() => handleRiskTypeChange(option.value)}
        />
        {option.label}
      </label>
    ))}
        ...
        <div style={{ padding: '16px 28px' }}>
          ...
          {/* Column headers */}
          <div style={{ display: 'grid', gridTemplateColumns: '80px 100px 1fr 120px 110px 120px', padding: '8px 0', borderBottom: '0.5px solid #e0ddd5', marginBottom: '4px' }}>
          ...
          {loading ? (
            ...
            <div style={{ fontSize: '13px', color: '#888780', padding: '24px 0', textAlign: 'center' }}>
              No early warnings yet. Click + New EWN to add one.
            </div>
          ) : (
            warnings.map(ew => {
              const sc = statusColors[ew.status] || statusColors.open
              return (
                <div key={ew.id} style={{ display: 'grid', gridTemplateColumns: '80px 100px 1fr 120px 110px 120px', padding: '12px 0', borderBottom: '0.5px solid #f1efe8', alignItems: 'center' }}>
                  <span style={{ fontWeight: 500, fontSize: '13px' }}>{ew.reference}</span>
  ...
  {Array.isArray(ew.risk_type) 
    ? ew.risk_type.map(r => riskTypeLabels[r]).join(', ') 
    : riskTypeLabels[ew.risk_type]}
</span>
                  <span>
                    <select
                      value={ew.status}
                      onChange={e => updateStatus(ew.id, e.target.value)}
