...
function Documents({ ceId, ewId, paymentId, subcontractId, inline, onClose }) {
    ...
    <div style={{ background: '#f9f8f5', border: '0.5px solid #e0ddd5', borderRadius: '12px', padding: '24px', marginBottom: '24px' }}>
      <form onSubmit={handleUpload}>
        ...
        <div style={fieldStyle}>
          ...
          <input type="file" onChange={handleFileChange} required accept=".pdf,.doc,.docx,.xls,.xlsx,.msg,.eml,.jpg,.png" style={{ ...inputStyle, padding: '6px 12px' }} />
          <div style={{ fontSize: '11px', color: '#888780', marginTop: '4px' }}>Accepted: PDF, Word, Excel, Outlook, images</div>
        </div>
        <div style={fieldStyle}>
        ...
        <div style={{ display: 'flex', justifyContent: 'flex-end' }}>
          <button type="submit" disabled={uploading} style={{ padding: '8px 20px', fontSize: '13px', fontWeight: 500, background: '#2c2c2a', color: '#fff', border: 'none', borderRadius: '6px', cursor: uploading ? 'not-allowed' : 'pointer', opacity: uploading ? 0.7 : 1 }}>
  ...
  const docList = (
    <div style={{ background: '#fff', border: '0.5px solid #e0ddd5', borderRadius: '12px', overflow: 'hidden' }}>
      {loading ? (
        ...
        <div style={{ padding: '40px', textAlign: 'center', color: '#888780', fontSize: '14px' }}>No documents uploaded yet.</div>
      ) : (
        documents.map(doc => (
          <div key={doc.id} style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '12px 20px', borderBottom: '0.5px solid #f1efe8' }}>
            <div>
              <div style={{ fontSize: '13px', fontWeight: 500, color: '#2c2c2a', marginBottom: '3px' }}>{doc.title}</div>
              <div style={{ display: 'flex', gap: '12px' }}>
                <span style={{ fontSize: '11px', background: '#f1efe8', padding: '2px 8px', borderRadius: '4px', color: '#5f5e5a' }}>{documentTypeLabels[doc.document_type]}</span>
                <span style={{ fontSize: '11px', color: '#888780' }}>{new Date(doc.created_at).toLocaleDateString('en-GB')}</span>
                {doc.notes && <span style={{ fontSize: '11px', color: '#888780' }}>{doc.notes}</span>}
              </div>
            </div>
            <div style={{ display: 'flex', gap: '8px' }}>
              <button onClick={() => handleDownload(doc)} style={{ fontSize: '12px', padding: '5px 12px', borderRadius: '6px', border: '0.5px solid #b4b2a9', background: '#fff', cursor: 'pointer' }}>Open</button>
              <button onClick={() => handleDelete(doc)} style={{ fontSize: '12px', padding: '5px 12px', borderRadius: '6px', border: '0.5px solid #fcebeb', background: '#fcebeb', cursor: 'pointer', color: '#a32d2d' }}>Delete</button>
            </div>
          </div>
        ))
      )}
