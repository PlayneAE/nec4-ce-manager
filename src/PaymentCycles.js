...
function PaymentCycles({ onClose, userRole }) {
    ...
    <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.4)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 100 }}>
      <div style={{ background: '#fff', borderRadius: '12px', width: '100%', maxWidth: '900px', maxHeight: '90vh', overflowY: 'auto' }}>
        ...
        <div style={{ padding: '16px 28px' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '60px 100px 110px 110px 110px 110px 120px 110px', padding: '8px 0', borderBottom: '0.5px solid #e0ddd5', marginBottom: '4px' }}>
          ...
          {loading ? (
            <div style={{ fontSize: '13px', color: '#888780', padding: '24px 0', textAlign: 'center' }}>Loading...</div>
          ) : cycles.length === 0 ? (
            <div style={{ fontSize: '13px', color: '#888780', padding: '24px 0', textAlign: 'center' }}>
              No payment cycles yet. Click + New cycle to add one.
            </div>
          ) : (
            cycles.map(cycle => {
              ...
              return (
                <div key={cycle.id} style={{ display: 'grid', gridTemplateColumns: '60px 100px 110px 110px 110px 110px 120px 110px', padding: '12px 0', borderBottom: '0.5px solid #f1efe8', alignItems: 'center' }}>
                  ...
                  <span style={{ fontSize: '13px', color: '#3d3d3a' }}>{new Date(cycle.assessment_date).toLocaleDateString('en-GB')}</span>
                  <span style={{ fontSize: '13px', color: '#3d3d3a' }}>
                    {cycle.afp_amount ? `£${Number(cycle.afp_amount).toLocaleString('en-GB')}` : '—'}
                  </span>
                  <span style={{ fontSize: '13px', color: '#3d3d3a' }}>
                    {cycle.certified_amount ? `£${Number(cycle.certified_amount).toLocaleString('en-GB')}` : '—'}
                  </span>
                  <span style={{ fontSize: '13px', color: '#3d3d3a' }}>
                    {cycle.pay_less_notice_amount ? `£${Number(cycle.pay_less_notice_amount).toLocaleString('en-GB')}` : '—'}
                  </span>
                  ...
                  <span>
                    <select
                      value={cycle.status}
                      onChange={e => updateStatus(cycle.id, e.target.value)}
                      style={{ ...sc, fontSize: '11px', fontWeight: 500, padding: '3px 6px', borderRadius: '4px', border: 'none', cursor: 'pointer' }}
                    >
